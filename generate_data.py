...
def fetch_and_store_data(query, max_results=1000):
    for result in h.search(query, per_page=100, pages=max_results//100):
        for host in result:
            ...
            cursor.execute('''
            INSERT OR REPLACE INTO hosts (ip, country, asn, ports, services)
            VALUES (?, ?, ?, ?, ?)
            ''', (ip, country, asn, ports, services))
...
fetch_and_store_data("services.port: 80 OR services.port: 443")
